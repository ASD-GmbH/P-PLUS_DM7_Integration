...
{
    [TestFixture]
    public sealed class API_Version_2__Schicht_1 : API_Test_Base
    {
        protected override void Erzeuge_Infrastruktur(int auswahllistenversion)
        {
            ...
            Setup_Testframework(host.Schicht_1_API_Version_3, server);
        }
    }

    [TestFixture]
    public class API_Version_2__Schicht_2 : API_Test_Base
    {
        protected override void Erzeuge_Infrastruktur(int auswahllistenversion)
        {
            var server = new Test_PPLUS_Backend(auswahllistenversion);
            ...
            var host = DM7_PPLUS_Host.Starten(server, new StaticAuthentication("test"), log, ex => Assert.Fail(ex.ToString()));
            var proxy = TestConnector.Instance_API_Version_3("test://test", new TestLog("[client] "), CancellationToken.None, new LoopbackFactory(host, 2)).Result;
    ...
    [TestFixture]
    public class API_Version_2__Schicht_3 : API_Test_Base
    {
        protected override void Erzeuge_Infrastruktur(int auswahllistenversion)
        {
            var server = new Test_PPLUS_Backend(auswahllistenversion);
            ...
            var host = DM7_PPLUS_Host.Starten(server, new StaticAuthentication("test"), log, ex => Assert.Fail(ex.ToString()));
            var proxy = TestConnector.Instance_API_Version_3("test://test", new TestLog("[client] "), CancellationToken.None, new LoopbackFactory(host, 3)).Result;
    ...
    [TestFixture]
    public class API_Version_2__Schicht_4 : API_Test_Base
    {
    ...
    public abstract class API_Test_Base
    {
        ...
        protected void Setup_Testframework(DM7_PPLUS_API version_2_API, Test_PPLUS_Backend server)
        {
            API = version_2_API ?? throw new ArgumentNullException("version_2_API nicht initialisiert!");
            _server = server;
        }
